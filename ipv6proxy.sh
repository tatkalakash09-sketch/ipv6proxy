...
# --- Defaults ---
subnet="auto"              # now auto by default
proxies_type="http"
start_port=30000
rotating_interval=0
use_localhost=false
...
# --- Logging helpers ---
log_err(){ echo "$1" 1>&2; echo -e "$1\n" &>> "$script_log_file"; }
log_err_and_exit(){ log_err "$1"; exit 1; }
log_err_print_usage_and_exit(){ log_err "$1"; usage; }

# --- Utils ---
is_valid_ip(){ [[ "$1" =~ ^(([0-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])$ ]]; }
...
last_port=0   # set after args validation

# --- State checks ---
is_proxyserver_installed(){ [[ -d "$proxy_dir" && -n "$(ls -A "$proxy_dir" 2>/dev/null)" ]]; }
is_proxyserver_running(){ pgrep -f "$proxyserver_config_path" >/dev/null 2>&1; }
is_package_installed(){ dpkg-query -W -f='${Status}' "$1" 2>/dev/null | grep -q "ok installed"; }
is_ndppd_running(){ ps aux | grep -q "[n]dppd"; }
...
check_startup_parameters(){
  ...
  # Subnet: allow "auto" or numeric divisible by 4
  if [[ "$subnet" != "auto" ]]; then
    ...
    (( subnet % 4 == 0 )) || log_err_print_usage_and_exit "Error: --subnet must be divisible by 4 (nibble aligned)"
  fi
  ...
  if cat "/sys/class/net/$interface_name/operstate" 2>&1 | grep -q "No such file or directory"; then
    log_err_print_usage_and_exit "Error: bad interface '$interface_name'"
  fi
}

# --- System checks & setup helpers ---
kill_3proxy(){ pgrep -f "[3]proxy" | xargs -r kill; }
...
install_package(){
  if ! is_package_installed "$1"; then
    apt-get update -y &>> "$script_log_file" || true
    apt-get install -y "$1" &>> "$script_log_file" || log_err_and_exit "Error: cannot install package '$1'"
  fi
}
...
check_ipv6(){
  ...
  if ! ping -6 -c 1 -W 2 google.com >/dev/null 2>&1; then
    log_err_and_exit "Error: IPv6 connectivity check failed"
  fi
}
...
configure_ipv6(){
  ...
  for option in "${opts[@]}"; do
    ...
    grep -v "#" /etc/sysctl.conf | grep -q "^$line$" || echo "$line" >> /etc/sysctl.conf
  done
  sysctl -p &>> "$script_log_file" || true
  [[ "$(cat /proc/sys/net/ipv6/conf/$interface_name/proxy_ndp)" == "1" && "$(cat /proc/sys/net/ipv6/ip_nonlocal_bind)" == "1" ]] \
...
autodetect_subnet_and_mask(){
  # Get first global IPv6 on interface
  local ip_with_prefix
  ip_with_prefix=$(ip -6 addr show dev "$interface_name" scope global | awk '/inet6/ {print $2; exit}')
  [[ -n "$ip_with_prefix" ]] || log_err_and_exit "Error: cannot detect a global IPv6 on $interface_name"
  local full_ip="${ip_with_prefix%/*}"
  local prefixlen="${ip_with_prefix#*/}"

  # Floor to nibble alignment (multiple of 4) for address generation
  local nibble_aligned=$(( prefixlen - (prefixlen % 4) ))
  if [[ "$subnet" == "auto" ]]; then
    subnet=$nibble_aligned
  fi

  # Build subnet_mask string from detected IP and chosen subnet
  local full_blocks_count=$(( subnet / 16 ))
  subnet_mask=$(echo "$full_ip" | grep -m1 -oP '^(?!fe80)([0-9a-fA-F]{1,4}:){'$(($full_blocks_count-1))'}[0-9a-fA-F]{1,4}')
  if (( subnet % 16 != 0 )); then
    local next_block
    next_block=$(echo "$full_ip" | awk -v block=$((full_blocks_count + 1)) -F ':' '{print $block}' | tr -d ' ')
    while ((${#next_block} < 4)); do next_block="0$next_block"; done
    local nibbles=$(( (subnet % 16) / 4 ))
    subnet_mask="$subnet_mask:$(echo "$next_block" | head -c "$nibbles")"
  fi
}
...
is_ndppd_routing_working(){
  ...
  curl -m 5 -s --interface "$test_ip" https://ipv6.ip.sb | grep -q "$test_ip" && return 0 || true
  curl -m 5 -s --interface "$test_ip" https://whatismyv6.com | grep -q "$test_ip" && return 0 || true
  curl -m 5 -s --interface "$test_ip" http://ip6only.me | grep -q "$test_ip" && return 0 || true
  curl -m 5 -s --interface "$test_ip" https://myipv6.is | grep -q "$test_ip" && return 0 || true
  curl -m 5 -s --interface "$test_ip" https://dnschecker.org/whats-my-ip-address.php | grep -q "$test_ip" && return 0 || true
...
close_ufw_backconnect_ports(){
  if ! is_package_installed "ufw" || ${use_localhost} || [[ ! -f "$backconnect_proxies_file" ]]; then return; fi
...
open_ufw_backconnect_ports(){
  ...
  if ! is_package_installed "ufw"; then echo "Firewall not installed; ports considered open."; return; fi
...
get_truncated_subnet_mask(){
  ...
  mask="$subnet_mask"
  echo \${mask:0:\$((\${#subnet_mask}-trunc_symbols_count))}
}
...
EOSH
  ...
  if [[ "$rotate_every_request" = true ]]; then
    access_rules_part="${access_rules_part}
...
if [[ $can_route_via_ndppd -ne 0 ]]; then
...
for pid in "${proxyserver_process_pids[@]}"; do kill "$pid" 2>/dev/null || true; done
...
add_to_cron(){
  ...
  crontab -l 2>/dev/null | grep -v "$startup_script_path" >> "$cron_script_path" || true
...
run_proxy_server(){
  ...
  if is_proxyserver_running; then
    ...
    echo -e "\nIPv6 proxy server started: $backconnect_ipv4:$start_port$creds .. $backconnect_ipv4:$last_port$creds ($proxies_type)"
    echo "Proxy list: $backconnect_proxies_file"
  else
...
write_backconnect_proxies_to_file(){
  ...
  if ! touch "$backconnect_proxies_file" &>> "$script_log_file"; then
    log_err "Warning: invalid path for proxy list file"
    return
  fi
  local line_creds=""
  local count=0
  ...
  if [[ "$use_random_auth" = true ]]; then
    ...
    line_creds=":$user:$password"
  fi
  for port in $(seq "$start_port" "$last_port"); do
    if [[ "$use_random_auth" = true ]]; then
      echo "$backconnect_ipv4:$port:${proxy_random_credentials[$count]}" >> "$backconnect_proxies_file"
      ((count+=1))
    else
      echo "$backconnect_ipv4:$port$line_creds" >> "$backconnect_proxies_file"
    fi
  done
}
...
# Auto-detect subnet/mask if needed (and always compute mask)
autodetect_subnet_and_mask
